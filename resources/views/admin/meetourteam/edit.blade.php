...
<main class="h-full pb-16 overflow-y-auto">
    <div class="container px-6 mx-auto grid">
      ...
      <form action="{{route('meetourteam.update',$meetourteam->id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div
        ...
        <label class="mt-6 block text-sm">
            ...
            <input
              ...
              name="position_name"
              value="{{$meetourteam->position_name}}"
            />
        ...
        <label class="block mt-6 text-sm">
            ...
            <textarea
              ...
              name="description"
            >{{$meetourteam->description}}</textarea>
            @if($errors->has('description'))
            <span class="text-xs text-red-600 dark:text-red-400">{{$errors->first('description')}}</span>
            @endif
        </label>

        <label class="mt-4 block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="user_avatar">Upload Image</label>
